...
import kryptonbutterfly.l4j.ui.search.SearchGui;
import kryptonbutterfly.l4j.ui.search.SearchKeyData;
import kryptonbutterfly.l4j.ui.settings.Settings;
import kryptonbutterfly.l4j.util.DocumentListenerAdapter;
import kryptonbutterfly.l4j.util.KeyEventListener;
import kryptonbutterfly.l4j.util.KeyEventType;
import kryptonbutterfly.l4j.util.Sneaky;
...
class BusinessLogic extends Logic<MainGui, Localizer>
{
	...
	{
		gui.if_(
			gui -> project.if_(project -> new Settings(gui, ModalityType.APPLICATION_MODAL, event ->
			{}, project.fileSettings, l10n)));
	}
	
	void showGeneralOptions(ActionEvent ae)
	{
		gui.if_(gui -> new Settings(gui, ModalityType.APPLICATION_MODAL, event -> {}, l10n));
	}
